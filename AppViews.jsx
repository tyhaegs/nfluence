  ...
  return (
    <div style={{ minHeight: "100vh", overflowX: "hidden", background: "radial-gradient(circle at calc(46% + 250px) calc(58% - 175px), rgba(255,255,255,.103) 0%, rgba(255,255,255,.0309) 38%, transparent 52%), linear-gradient(180deg, #040b15 0%, #070f1f 100%)", backgroundColor: "#040b15", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px 0", position: "relative" }}>
        <div style={{ width: "100%", height: 160, borderRadius: 16, overflow: "hidden", background: "linear-gradient(135deg, rgba(100,80,255,.3), rgba(255,80,160,.18))" }} />
      ...
      {showBrandEdit && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", backdropFilter: "blur(8px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={() => setShowBrandEdit(false)}>
          <div style={{ background: "#0a1020", border: "1px solid rgba(255,255,255,.15)", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 460, maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ fontFamily: "'Monda', system-ui, sans-serif", fontSize: "1.1rem", fontWeight: 700, marginBottom: 24 }}>edit profile</div>

            {/* Logo upload */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: ".75rem", opacity: .4, marginBottom: 8, textTransform: "lowercase" }}>logo</div>
              <label style={{ display: "flex", alignItems: "center", gap: 14, cursor: "pointer" }}>
                <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) { const reader = new FileReader(); reader.onload = ev => setBrandEditForm(f => ({ ...f, logoPreview: ev.target.result })); reader.readAsDataURL(file); }
                }} />
                <div style={{ width: 64, height: 64, borderRadius: 14, background: brandEditForm.logoPreview ? "transparent" : "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.15)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0 }}>
                  {brandEditForm.logoPreview ? <img src={brandEditForm.logoPreview} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: ".65rem", opacity: .4 }}>upload</span>}
                </div>
                <div style={{ fontSize: ".8rem", opacity: .4 }}>click to upload brand logo</div>
              </label>
            </div>

            {/* Banner upload */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: ".75rem", opacity: .4, marginBottom: 8, textTransform: "lowercase" }}>banner</div>
              <label style={{ display: "block", cursor: "pointer" }}>
                <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) { const reader = new FileReader(); reader.onload = ev => setBrandEditForm(f => ({ ...f, bannerPreview: ev.target.result })); reader.readAsDataURL(file); }
                }} />
                <div style={{ width: "100%", height: 80, borderRadius: 12, background: brandEditForm.bannerPreview ? "transparent" : "rgba(255,255,255,.04)", border: "1px dashed rgba(255,255,255,.15)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                  {brandEditForm.bannerPreview ? <img src={brandEditForm.bannerPreview} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: ".8rem", opacity: .35 }}>click to upload banner image</span>}
                </div>
              </label>
            </div>

            {/* Text fields */}
            {[["brand name", "name"], ["website", "website"]].map(([label, key]) => (
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: ".75rem", opacity: .4, marginBottom: 5, textTransform: "lowercase" }}>location</div>
              <div style={{ display: "flex", gap: 8 }}>
                <input style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "#fff", padding: "8px 10px", fontSize: ".85rem", flex: 2, outline: "none", fontFamily: "system-ui, sans-serif" }}
                  value={brandEditForm.city || ""} onChange={e => setBrandEditForm(f => ({ ...f, city: e.target.value }))} placeholder="city" />
                <select style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "#fff", padding: "8px 10px", fontSize: ".85rem", flex: 1, outline: "none", fontFamily: "system-ui, sans-serif", appearance: "none" }}
                  value={brandEditForm.state || ""} onChange={e => setBrandEditForm(f => ({ ...f, state: e.target.value }))}>
                  <option value="">state</option>
                  {US_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <input style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "#fff", padding: "8px 10px", fontSize: ".85rem", flex: 1, outline: "none", fontFamily: "system-ui, sans-serif" }}
                  value={brandEditForm.country || ""} onChange={e => setBrandEditForm(f => ({ ...f, country: e.target.value }))} placeholder="country" />
              </div>
            </div>

            {/* Industry */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: ".75rem", opacity: .4, marginBottom: 5, textTransform: "lowercase" }}>industry</div>
              <select style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "#fff", padding: "8px 10px", fontSize: ".85rem", width: "100%", outline: "none", fontFamily: "system-ui, sans-serif", appearance: "none" }}
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: ".75rem", opacity: .4, marginBottom: 5, textTransform: "lowercase" }}>bio</div>
              <textarea style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "#fff", padding: "8px 10px", fontSize: ".85rem", width: "100%", outline: "none", fontFamily: "system-ui, sans-serif", minHeight: 80, resize: "vertical" }}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { const location = [brandEditForm.city, brandEditForm.state, brandEditForm.country].filter(Boolean).join(", "); onUpdateUser?.({ ...user, ...brandEditForm, location }); setShowBrandEdit(false); }} style={{ flex: 1, padding: "10px", borderRadius: 12, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.08)", color: "#fff", cursor: "pointer", fontFamily: "system-ui, sans-serif", fontSize: ".9rem" }}>save</button>
            </div>
          </div>
        </div>
      )}
