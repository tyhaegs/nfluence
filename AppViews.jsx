              ...
              {headerTotal > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8 }}>
                  <div style={{ display: "flex", gap: 1 }}>
                    {[1,2,3,4,5].map(i => <span key={i} style={{ fontSize: ".75rem", color: i <= Math.floor(headerAvg) ? "#fbbf24" : "rgba(251,191,36,.2)" }}>★</span>)}
                  </div>
                  <span onClick={onViewReviews} style={{ fontSize: ".8rem", opacity: .5, textDecoration: "underline", textDecorationColor: "rgba(255,255,255,.25)", cursor: "pointer" }}>{headerTotal} review{headerTotal !== 1 ? "s" : ""}</span>
                </div>
              )}
            </div>
          );
                    ...
                    <div style={{ flex: 1 }}>
                      {c.deadline && (
                        <div style={{ display: "flex", gap: 8, padding: "3px 0", fontSize: ".95rem" }}>
                          <div style={{ opacity: .35, minWidth: 80 }}>deadline:</div>
