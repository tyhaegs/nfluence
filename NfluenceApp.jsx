...
function NfluenceApp() {
  ...
  useEffect(() => {
    ...
    sb.auth.getSession().then(async ({ data: { session }, error }) => {
      ...
      // Replay onboarding profile stashed before email confirmation, BEFORE the role branches'
      // profile reads run — so a user returning via the confirmation-link auto-session (which
      // never hits the manual sign-in handlers) gets their stashed onboarding data written, and
      // the subsequent bare-profile read sees it instead of racing ahead. Mirrors handleSignIn /
      // handleCreatorSignIn. Guarded on role+email so it can't replay onto the wrong account;
      // on error the stash is left in place so a later manual sign-in can still replay it. (B1)
      const pending = readPendingProfile();
      if (pending && pending.role === role && (pending.email || '').toLowerCase() === (u.email || '').toLowerCase()) {
        const d = pending.data || {};
        if (role === 'brand') {
          const { error: rErr } = await sb.from('profiles').upsert({
          ...
          if (rErr) { console.error('[pending-profile] brand replay (bootstrap) failed:', rErr); }
          else {
            clearPendingProfile();
            setUser(prev => ({ ...prev,
          ...
          const { error: pErr } = await sb.from('profiles').upsert({ id: u.id, email: u.email, name: d.name || name, role: 'creator' });
          const { error: cErr } = await sb.from('creator_profiles').upsert({
          ...
          if (pErr || cErr) { console.error('[pending-profile] creator replay (bootstrap) failed:', pErr || cErr); }
          else {
            clearPendingProfile();
            setCreatorProfile(prev => ({ ...prev, ...d, name: d.name || name,
  ...
  const handleCreatorSignIn = async (email, name, password, profileData = null) => {
    ...
    if (_sb) {
      ...
      if (error && profileData && error.message?.includes('Invalid login credentials')) {
        ...
        if (signUpData?.user) {
          { const { error: pErr } = await _sb.from('profiles').upsert({ id: signUpData.user.id, email, name, role: 'creator' }); if (pErr) console.error('[creator signup] profiles upsert:', pErr); }
          { const { error: cErr } = await _sb.from('creator_profiles').upsert({ id: signUpData.user.id }); if (cErr) console.error('[creator signup] creator_profiles upsert:', cErr); }
          setAuthSession(signUpData.session);
