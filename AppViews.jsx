        ...
        {(() => {
          return (
            <div style={{ paddingTop: 64, marginBottom: 24 }}>
              ...
              {user?.website && <a href={user.website.startsWith("http") ? user.website : `https://${user.website}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: ".82rem", opacity: .4, color: "rgba(255,255,255,.4)", textDecoration: "underline", display: "inline-block", marginTop: 3 }}>{user.website}</a>}
              {user?.bio && <div style={{ fontSize: ".85rem", opacity: .55, marginTop: 6, lineHeight: 1.6, maxWidth: "min(520px, 100%)" }}>{user.bio}</div>}
              {(() => {
